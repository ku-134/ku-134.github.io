...
const UI = (() => {
  ...
  const QUEST_GUIDES = {
    ...
    q_leave:   '太空中按 <b>M</b> 打开星系地图 → 点击任意星系 → 曲速跃迁，完成第一章！',
  };
  ...
  // ---------- 星系地图（NMS 式 3D 星图：自由旋转/缩放/点选，航线连线）----------
  ...
  function buildGalaxyMap(){
    ...
    $('galWarpInfo').textContent = `曲率电池 ×${wc} · 拖动旋转 · 滚轮缩放 · 点击选择`;
    $('galInfo').innerHTML = '<div class="save-empty">— 点击星系查看详情 —</div>';
    ...
    const seenSeeds = new Set([cur, HOME_GALAXY_SEED]);
    ...
    for (let i = 0; i < 12; i++){
      const s = seedList[i]; seenSeeds.add(s);
      const a = i / 12 * Math.PI * 2, r = 24 + (i % 4) * 10, y = (Math.sin(i * 2.3) * 16);
    ...
    for (let ring = 0; ring < 4; ring++){
      for (let i = 0; i < 12; i++){
        ...
        seenSeeds.add(s);
        const a = (i / 12 + ring * 0.07) * Math.PI * 2;
        const r = 44 + ring * 16 + (i % 4) * 6;
        const y = (ring - 1.5) * 18 + Math.sin(i * 1.7) * 8;
        entries.push({ seed: s, pos: new THREE.Vector3(Math.cos(a) * r, y, Math.sin(a) * r) });
      }
    }
    if (!g3d){
      ...
      g3d = { renderer, yaw: 0.6, pitch: 0.33, dist: 135, drag: null, labels: [], raf: 0, stars: [], entries: [] };
      bindGalaxyControls();
    ...
    // 背景星海
    {
      ...
      for (let i = 0; i < 700; i++){
        const v = new THREE.Vector3(rnd() * 2 - 1, (rnd() * 2 - 1) * 0.55, rnd() * 2 - 1).normalize().multiplyScalar(180 + rnd() * 320);
        pos.push(v.x, v.y, v.z);
      }
      const geo = new THREE.BufferGeometry();
      geo.setAttribute('position', new THREE.Float32BufferAttribute(pos, 3));
      scene.add(new THREE.Points(geo, new THREE.PointsMaterial({ color: 0x8899bb, size: 1.6, sizeAttenuation: false, transparent: true, opacity: 0.75 })));
    }
    // 星云辉光
    const nebCols = ['rgba(53,224,232,0.26)', 'rgba(180,140,255,0.22)', 'rgba(255,140,90,0.18)'];
    for (let i = 0; i < 3; i++){
      const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: starTexture(nebCols[i]), transparent: true, opacity: 0.55, depthWrite: false }));
      const rnd = mulberry32(999 + i * 77);
      sp.position.set((rnd() - 0.5) * 230, (rnd() - 0.5) * 90, (rnd() - 0.5) * 230);
      sp.scale.setScalar(210 + rnd() * 170);
      scene.add(sp);
    }
    // 星系恒星 + 航线 + 标签
    g3d.stars = [];
    const texCur = starTexture('#7ff5fa'), texHome = starTexture('#b48cff'), texStd = starTexture('#ffd97a');
    const linePos = [];
    for (const ent of entries){
      const spr = new THREE.Sprite(new THREE.SpriteMaterial({
        map: ent.current ? texCur : ent.home ? texHome : texStd, transparent: true, depthWrite: false }));
      spr.position.copy(ent.pos);
      spr.scale.setScalar(ent.current ? 12 : 8.5);
      ...
      el.textContent = (ent.current ? '⬤ ' : '') + (ent.seed === HOME_GALAXY_SEED ? '起源星系' : galaxyName(ent.seed));
      box.appendChild(el);
      g3d.labels.push(el);
      ent.label = el;
    }
    ...
    scene.add(new THREE.LineSegments(lgeo, new THREE.LineBasicMaterial({ color: 0x35e0e8, transparent: true, opacity: 0.22 })));
    // 选中环
    g3d.ring = new THREE.Sprite(new THREE.SpriteMaterial({ map: ringTexture(), transparent: true, depthWrite: false }));
    g3d.ring.scale.setScalar(15);
    g3d.ring.visible = false;
    scene.add(g3d.ring);
    g3d.entries = entries;
    resizeGalaxy();
  ...
  function bindGalaxyControls(){
    ...
    el.addEventListener('wheel', e => {
      e.preventDefault();
      g3d.dist = THREE.MathUtils.clamp(g3d.dist * (e.deltaY > 0 ? 1.12 : 0.89), 45, 420);
    }, { passive: false });
  }
  function pickGalaxy(e){
    ...
    ray.setFromCamera(m, g3d.cam);
    const hits = ray.intersectObjects(g3d.stars);
    if (!hits.length) return;
    const ent = hits[0].object.userData.ent;
    Sound.play('uiClick');
    galSelected = ent;
    g3d.ring.visible = true;
    g3d.ring.position.copy(ent.pos);
    g3d.labels.forEach(l => l.classList.remove('sel'));
    if (ent.label) ent.label.classList.add('sel');
    const gal = ent.seed === HOME_GALAXY_SEED
      ? { name: '起源星系', planets: DEFAULT_PLANETS }
      : generateGalaxy(ent.seed);
    showGalaxyDetail(ent, gal);
  }
  ...
  function galaxyTick(){
    ...
    g3d.cam.position.set(
      Math.cos(g3d.pitch) * Math.sin(g3d.yaw) * g3d.dist,
      Math.sin(g3d.pitch) * g3d.dist,
      Math.cos(g3d.pitch) * Math.cos(g3d.yaw) * g3d.dist);
    g3d.cam.lookAt(0, 0, 0);
    ...
    for (const ent of g3d.entries){
      ...
      if (_gV.z > 1){ ent.label.style.display = 'none'; continue; }
      ent.label.style.display = '';
      ent.label.style.left = ((_gV.x + 1) / 2 * w) + 'px';
      ent.label.style.top = ((1 - _gV.y) / 2 * h + 8) + 'px';
    }
  }
  function showGalaxyDetail(ent, gal){
    const info = $('galInfo');
    const wc = Player.countItem('warpcell');
    const cur = ent.current;
    let html = `<div class="gal-detail"><h3>✦ ${gal.name}</h3><div class="gd-seed">星图坐标 #${ent.seed}</div>`;
    ...
    if (!cur){
      btn.textContent = locked ? '◉ 解除锁定' : (wc >= 1 ? '◎ 锁定星系（出图后对准方框按 K 跃迁）' : '◎ 锁定导航方向（需要 1 枚曲率电池才可跃迁）');
      btn.onclick = () => {
        Game.setWarpLock(locked ? null : ent.seed, gal.name);
        showGalaxyDetail(ent, gal);   // 刷新按钮态
      };
    }
