...
// 战场干扰球：每局随机一个（巨人 | 魔王 | 太阳 | 水星），均为第三方，不参与胜负
...
const WILD_IDS = ['giant', 'demon', 'sun', 'mercury'];   // 水星=星球战场球（2倍体型轨道流）
...
// 创建战场干扰球（巨人 r=20 红色暴怒 / 魔王 r=30 紫色召唤 / 太阳 r=60 燃烧 / 水星 r=40 轨道公转）
export function makeWildBall(id, ctx, w, h) {
  const isDemon = id === 'demon';
  const isSun = id === 'sun';
  const isMercury = id === 'mercury';
  const b = new Ball({
    x: w * 0.5, y: isDemon ? h * 0.65 : isSun ? h * 0.5 : isMercury ? h * 0.3 : h * 0.35,
    angle: Math.random() * Math.PI * 2,
    hp: CONFIG.WILD.hp,
    radius: isDemon ? CONFIG.BALL.radius * CONFIG.DEMON.scale
      : isSun ? CONFIG.BALL.radius * CONFIG.SUN.scale
      : isMercury ? CONFIG.BALL.radius * 2 : undefined,   // 水星：2倍体型
    name: isDemon ? '战场魔王' : isSun ? '战场太阳' : isMercury ? '战场水星' : '战场巨人',
  });
...
export class SingleMode {
  ...
  start(playerSkillId, aiSkillId = 'legion', battleId = 'arena') {
    ...
    p1.isPlayer = true;
    // 战场干扰球：每局随机一个（巨人 | 魔王 | 太阳 | 水星），出生自场地点位
