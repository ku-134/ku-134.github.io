...
const Game = (() => {
  ...
  let skyPlanets = [], nightStars = null, planetSun = null, skyStation = null;
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.code === 'KeyK' && state === 'space' && !UI.anyPanelOpen()){ attemptWarpJump(); }
  ...
  function setWarpLock(seed, name){
    ...
    if (warpLock) UI.bigMessage('◎ 已锁定 ' + warpLock.name, '退出星图 · 对准太空中的方框标记按 K 跃迁（消耗曲率电池×1）', 3600);
  }
  ...
  function updateWarpMarker(){
    ...
    if (!warpBoxEl){
      ...
      warpBoxEl.innerHTML = '<span class="wb-tx"></span>';
    ...
    if (!behind && Math.abs(_proj.x) < 0.88 && Math.abs(_proj.y) < 0.84){
      ...
      warpBoxEl.querySelector('.wb-tx').textContent = `${warpLock.name} · ${galaxyLightYears(warpLock.seed)} 光年 · [K] 跃迁`;
    } else {
  ...
  function attemptWarpJump(){
    if (state !== 'space') return;
    if (!warpLock){ Sound.play('uiError'); UI.bigMessage('未锁定目标星系', '按 M 打开星系图 → 选择星系 → 锁定', 2400); return; }
    if (warpLock.seed === Space.getCurrentGalaxySeed()){ Sound.play('uiError'); UI.bigMessage('已在该星系', '', 1600); return; }
    if (Player.countItem('warpcell') < 1){
      Sound.play('uiError');
      UI.bigMessage('缺少曲率电池', '精炼厂合成或空间站购买', 2400);
      return;
    }
    const pos = Space.getGalaxySpritePos(warpLock.seed);
    if (!pos) return;
    ...
    if (camDir.dot(dir) < 0.94){
      Sound.play('uiError');
      UI.bigMessage('未对准目标', '将准星对准星系方框标记后按 K', 2000);
      return;
    }
    ...
    warpAnim = { t: 0, seed: warpLock.seed, yaw: Math.atan2(-dir.x, -dir.z), pitch: Math.asin(THREE.MathUtils.clamp(dir.y, -1, 1)), phase: 0, _f: 0 };
    warpLock = null;   // 跃迁开始即解除锁定标识（但动画仍持有 seed）
    UI.bigMessage('⟠ 曲速引擎点火', '航向锁定 · 全速冲刺', 3000);
  }
  let galaxyCount = 1;
  // 邻近星系种子（由当前星系种子确定性派生）
  ...
  // 构建水印：右下角常驻小字（station 态升级为实时仪表：阶段/相机/朝向逐帧显示）
  {
    ...
    bd.textContent = 'build v79-station';
  ...
  function loop(){
    ...
    if (window.__stDbg){
      ...
      __stDbg.textContent = 'v79 st:' + state + ' ph:' + ((window.Station && Station.phase) || '-') +
        ' f:' + __stFrames +
    ...
    if (state === 'planet'){
      ...
      // 速度：启航段 30→约 4800（EaseOutQuad 慢慢加速驶离），星轨段全速
      if (inLaunch){
        const ak = THREE.MathUtils.clamp(frame / 400, 0, 1);
        Space.shipState.speed = 30 + (1 - (1 - ak) * (1 - ak)) * 4770;
      } else {
